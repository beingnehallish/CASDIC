...
export function verifyToken(req, res, next) {
  ...
  jwt.verify(token, SECRET_KEY, (err, decoded) => {
    ...
    const id = decoded?.id ?? decoded?.user_id ?? decoded?.userId ?? decoded?.uid ?? null;
    const role = decoded?.role ?? decoded?.user_role ?? decoded?.type ?? null;

    if (!id) {
      return res.status(401).json({ error: "Unauthorized: id missing in token" });
    }
...
export function isEmployee(req, res, next) {
  if (req.user.role !== "employee") return res.status(403).json({ error: "Access denied: Employees only" });
  next();
}
