...
router.get("/", verifyToken, async (req, res) => {
  try {
    const [rows] = await db.promise().query(
      `SELECT project_id, name, tech_id, start_date, end_date, budget, description
       FROM projects`
    );
...
router.post("/", verifyToken, isEmployee, async (req, res) => {
  ...
  try {
    await db.promise().query(
      `INSERT INTO projects (name, description, start_date, end_date, budget, tech_id)
       VALUES (?, ?, ?, ?, ?, ?)`,
...
router.put("/:project_id", verifyToken, isEmployee, async (req, res) => {
  ...
  try {
    await db.promise().query(
      `UPDATE projects 
       SET name=?, description=?, start_date=?, end_date=?, budget=?, tech_id=?
       WHERE project_id=?`,
